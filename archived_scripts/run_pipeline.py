...
def run_preprocess():
    ...
    cmd_tilt_settings = [
        ...
        "--gain_path", os.path.join("frames", cfg.gain_ref),
        "--exposure", str(cfg.dose),
        "--tomo_dimensions", f"{cfg.original_x_y_size[0]}x{cfg.original_x_y_size[1]}x3000"
    ]
    ...
    with open("logs/stack.log", "w") as log_file:
        subprocess.run(cmd_ts_stack, check=True, stdout=log_file, stderr=subprocess.STDOUT)

    logging.info("Preprocessing stage completed.")

def run_custom_etomo():
...
def run_builtin_etomo():
    ...
    logging.info(f"Temporarily prepending to PATH for subprocess: {wrapper_dir}")
...
def run_postprocess():
    ...
    with open(rmd_error_log, "w") as f:
        for d in glob.glob("warp_tiltseries/tiltstack/L*"):
            align_log = os.path.join(d, "align_noRot2.log")
    ...
    original_dir = os.getcwd()
    try:
        os.chdir('warp_tiltseries/tiltstack')
        logging.info("Running XML parsing in warp_tiltseries/tiltstack")
        run_xml_parsing()
    finally:
        os.chdir(original_dir)
    ...
    with open("logs/deconv.log", "w") as log_file:
        for xml_file in glob.glob("warp_tiltseries/*.xml"):
            ...
            rec_file = f"{tomo_name}_rot_flipz.rec"
            ...
            for f in glob.glob(os.path.join(tomo_dir, "*~")):
                os.remove(f)
            for f in glob.glob(os.path.join(tomo_dir, "*/*rot.rec")):
                os.remove(f)
